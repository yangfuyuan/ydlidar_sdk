...
public:
    ...
    void LidarScanCallBack(const LaserScan &scan);

private:
    LaserParamCfg                   m_config;
    uint8_t                         m_type;
    std::shared_ptr<ydlidar::LIDAR> m_lidar;
};

LidarTest::LidarTest(const LaserParamCfg& config, uint8_t type) :
    m_config(config), m_type(type){
    m_lidar.reset(new ydlidar::LIDAR(type));
}
...
void LidarTest::LidarScanCallBack(const LaserScan &scan) {
...
void LidarTest::run() {
    ...
    try {
        m_lidar->UpdateLidarParamCfg(m_config);

        m_lidar->RegisterLIDARDataCallback(std::bind(&LidarTest::LidarScanCallBack, this, std::placeholders::_1));
...
int main(int argc, char **argv) {
    ...
    try {
        LidarTest test(cfg, driver_type);
        test.run();
